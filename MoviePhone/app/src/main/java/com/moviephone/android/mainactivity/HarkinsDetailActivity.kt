...
class HarkinsDetailActivity : AppCompatActivity() {
    ...
    private inner class Content :
        ...
        override fun doInBackground(vararg params: Void?): Void? {
            try {
                val baseUrl = "https://www.harkins.com"
                val detailUrl = intent.getStringExtra("detailUrl")
                val url = baseUrl + detailUrl
                movieURL = url
                val doc = Jsoup.connect(url).get()
                val data = doc.select("div.detail-list")
                val synopsisDiv = doc.select("div.synopsis")
                Log.d("Items", "URL$url")
                detailString =
                    "Starring: " + data.select("#movie-detail-starring-description").text()
                synopsis =
                    "Synopsis: " + synopsisDiv.select("#movie-detail-sidebar-synopsis-description").text()
                ageRating = "Rating: " + data.select("#movie-detail-rating-description").text()
            } catch (e: IOException) {
                e.printStackTrace()
            }
            return null
